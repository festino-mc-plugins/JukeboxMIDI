...
import com.festp.jukebox.JukeboxUtils;
...
public class NoteDisc {
		...
		for (Player player : soundSource.getWorld().getPlayers()) {
			if (player.getLocation().distance(soundSource) <= NoteUtils.SOUND_DISTANCE) {
				for (NoteSound sound : sounds) {
					...
					player.playSound(soundSource, sound.getSpigotSound(), SoundCategory.RECORDS, 3, pitch);
				}
			}
		}
		for (NoteSound sound : sounds) {
	...
	/** @return <b>null</b> if jukebox isn'useVanillaOctaves containing note disc */
